...
public class Room extends Obstacle {
	...
	public List<SignalArea> getAttenuatedAreas(AccessPoint ap, double panelWidth, double panelHeight) {
		...
		list.add(new SignalArea(attenuation, ProjectionHelper.getWallShadow(ap.getLocation(), wallTop, panelWidth, panelHeight)));
        list.add(new SignalArea(attenuation, ProjectionHelper.getWallShadow(ap.getLocation(), wallRight, panelWidth, panelHeight)));
        list.add(new SignalArea(attenuation, ProjectionHelper.getWallShadow(ap.getLocation(), wallLeft, panelWidth, panelHeight)));
        list.add(new SignalArea(attenuation, ProjectionHelper.getWallShadow(ap.getLocation(), wallBottom, panelWidth, panelHeight)));
		return list;
	}
}