...
public class test {
    ...
    public List<Wall> obstaclesToWalls(LinkedList<Obstacle> obstacles) {
    ...
    public double getAttenuation(Point2D pixel, AccessPoint ap, LinkedList<Wall> walls) {
        Line2D.Double line = new Line2D.Double(pixel.getX(), pixel.getY(), ap.getLocation().getX(), ap.getLocation()
                .getY());
        ...
        for (Wall w : walls) {
            if (line.intersectsLine((Line2D) w.getShape())) {
                attenuationMap.put(getIntersection(line, (Line2D.Double) w.getShape()), w.getMaterial()
